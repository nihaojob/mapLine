...
class Maps extends React.Component<ITextPaperProps, IEntranceState, any> {
    ...
    lineUtil(lineArr: any) {
        ...
        return {
            ...
            param :[ start.LT, end.LT, { waypoints: waypoints.length !== 0 ? waypoints.map(item => item.LT) : []}]
        }
    };
    ...
    // 获取导航数据
    getDrivLineData (option?: any, cb?:Function) {
    ...
    // 画线
    ployLine(path: any,strokeColor: string) {
        // 线路
        ...
        AMap.event.addListener(routeLine, 'mouseover', function (e: { target: { setOptions: (arg0: { strokeOpacity: number; }) => void; }; }) {
            e.target.setOptions({
                strokeOpacity:0.5
            })
        });

        AMap.event.addListener(routeLine, 'mouseout', function (e: { target: { setOptions: (arg0: { strokeOpacity: number; }) => void; }; }) {
            e.target.setOptions({
                strokeOpacity:1,
            });
        });
    ...
    addMarke (markArr: any[]) {
        ...
        function utliTmpl(markItem: { iconText: any; title: any; theme?: 1 | undefined; }) {
    ...
    // 画汽车动画
    carAnime(path: any,colorStr: string) {
        // 汽车图标
    ...
    initLintKeyTest(option: any) {

        const func = (path: any, source:any) => {
            const markArr = [source.start, ...source.waypoints, source.end].map( (item, index) => {
                return {
                    type:'仓',
    ...
    public render() {
        return (
            <div id={this.state.elId} style={{ width:'100%', height:'500px'}}>
            </div>
        );
    }
}
